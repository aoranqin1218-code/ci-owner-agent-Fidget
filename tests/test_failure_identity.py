...
def test_relative_and_var_app_paths_produce_same_identity():
    facts = [
        _path_fact("server/workflow/service.ts"),
        _path_fact("./server/workflow/service.ts"),
        _path_fact("/var/app/server/workflow/service.ts"),
        _path_fact("file:///var/app/server/workflow/service.ts"),
    ]
...
def test_dict_builder_normalizes_relative_and_var_app_path():
    relative = {
        "failureKind": "typescript_compile_error",
        "errorCode": "TS2305",
        "filePath": "server/workflow/service.ts",
    ...
    container = {**relative, "filePath": "/var/app/server/workflow/service.ts"}
    file_uri = {**relative, "filePath": "file:///var/app/server/workflow/service.ts"}

    assert len({build_failure_fact_signature(relative), build_failure_fact_signature(container), build_failure_fact_signature(file_uri)}) == 1


def test_unsupported_absolute_path_does_not_make_wrapper_eligible():
    fact = FailureFact(
        signatureKey="model-wrapper",
        historyEligible=True,
        isGenericWrapper=False,
...
def test_dict_builder_drops_unsupported_absolute_path():
    fact = {
        "failureKind": "build_failure",
        "filePath": "/var/lib/jenkins/workspace/services/fx-code-unittest/server/a.ts",
        "message": 'ERROR: process "/bin/sh -c npm run build" did not complete successfully: exit code: 1',
