...
INITIAL_INPUT_INSTRUCTION = (
    ...
    "investigationScope 描述调查范围：fullRange 是可信调查基线 -> currentBuild，focusRange 是 previousBuild -> currentBuild。"
    "integrationBaseline 非空时，版本检查点只限定集成 assertion 的调查范围，不是 owner 证据；"
    "high_confidence 仍必须以失败日志和相关 diff/代码证据证明，且 owner commit 必须落在 base..head。"
    "integrationSuiteBaselines 非空时，每个失败块的 integrationSuite 只能使用同名 suite 的范围；"
    "不得把一个 suite 的通过检查点当作另一个 suite 的成功证明。"
