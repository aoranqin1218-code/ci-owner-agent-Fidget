...
本期不建设支持多个仓库、多个 Jenkins、任意测试框架或任意通知渠道的通用适配平台，也不包含 Fidget Connection 测试接入、自动关闭测试或 Fidget 周报。Fidget 集成测试失败分析（`FIDGET_INTEGRATION_V1` 协议、环境失败 no-owner、代码/断言定责）已接入二期范围，详见 AGENTS.md「Fidget 二期当前代码基线」与 `samples/fidget_log/integration/PROTOCOL.md`。以下章节仍按当前源码描述现状；企业微信链路代表二期应优先复用的业务流程，但飞书 API、身份体系和交互形式的差异应封装在飞书专用边界中，不要求复制相同界面或建设任意渠道框架。
...
│   ├─ 通知反馈：wecom_notice_service / notification_formatter / wecom_notification_routing / wecom_notifier / outbox / bot_worker / feedback_*；飞书单向通知：feishu_notice_service / feishu_notification_formatter / feishu_notifier / feishu_user_mapping
...
| `ci_notifications`             | Webhook 直发记录与去重（企微 `channel=wecom` 与飞书 `channel=feishu` 各自隔离） | `(repo,job,branch,buildNumber,noticeHash,channel)` / status(sent/failed，**成功后永不降级**) / messagePreview / lastAttemptStatus                                          |
...
Fidget 集成 assertion 的调查范围由 [integration_baseline.py](ci_owner_agent/services/integration_baseline.py) 收敛：优先使用 `ci_builds.integrationRun` 中同 suite 的最近可信通过点，Mongo 历史没有可用记录时才解析 SDK 上一稳定版本；两者都只来自 Jenkins checkout 与 Agent 专用 Git cache，并且每项 owner 还要通过自己的 suite `baseline..head` 检查。`ci_builds` 只新增向后兼容的 `integrationRun`（协议状态、环境别名、suite exit）；旧文档会自然回退版本基线，不保存外部数据库连接信息。
